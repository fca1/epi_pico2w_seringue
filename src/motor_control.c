...
bool motor_config_valid(const motor_config_t*c){if(!c||!c->motor_steps_per_rev||c->screw_pitch_mm<=0||c->manual_speed_mm_s<=0||c->acceleration_mm_s2<=0)return false;uint16_t m=c->microsteps;return m<=256&&m&&!(m&(m-1));}
#ifndef UNIT_TEST
static uint32_t vel(float mm_s){return(uint32_t)(fabsf(mm_s)*motor_microsteps_per_mm(&cfg)*16777216.0f/12000000.0f);}
bool motor_init(const motor_config_t*c){if(!motor_config_valid(c))return false;cfg=*c;if(!tmc5130_init()||!tmc5130_configure(c->microsteps))return false;tmc5130_enable(true);return tmc5130_set_ramp(vel(c->manual_speed_mm_s),vel(c->acceleration_mm_s2),vel(c->acceleration_mm_s2));}
