...
safety_fault_t safety_check(safety_t*s,const device_config_t*c,bool both,bool timed_motion,bool stallguard_motion,uint32_t now,uint32_t ds,bool spi,float pos,int dir){
 ...
 if(timed_motion&&now-s->manual_started_ms>c->manual_timeout_ms)return s->fault=SAFETY_TIMEOUT;
 if((dir<0&&pos<=c->position_min_mm)||(dir>0&&pos>=c->position_max_mm))return s->fault=SAFETY_LIMIT;
 uint16_t sg=ds&0x3ffu;s->sg_filtered=s->sg_filtered?(uint16_t)((3u*s->sg_filtered+sg)/4u):sg;
 if(c->stallguard_enabled&&stallguard_motion){
