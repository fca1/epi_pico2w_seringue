...
static char uart_rx[112],uart_tx[96];static uint16_t uart_rx_len,uart_tx_len;static volatile bool uart_notify_pending;
static btstack_packet_callback_registration_t hci_event_callback;
static const uint8_t adv_data[]={2,BLUETOOTH_DATA_TYPE_FLAGS,0x06,17,BLUETOOTH_DATA_TYPE_COMPLETE_LIST_OF_128_BIT_SERVICE_CLASS_UUIDS,0x9e,0xca,0xdc,0x24,0x0e,0xe5,0xa9,0xe0,0x93,0xf3,0xa3,0xb5,0x01,0x00,0x40,0x6e};
...
static bd_addr_t static_random_address;
static uint16_t read_cb(hci_con_handle_t c,uint16_t handle,uint16_t offset,uint8_t*buffer,uint16_t size){(void)c;
 ...
 return 0;}
static void uart_reply(const char*text){size_t n=strlen(text);if(n>=sizeof(uart_tx))n=sizeof(uart_tx)-1;memcpy(uart_tx,text,n);uart_tx[n]=0;uart_tx_len=(uint16_t)n;uart_notify_pending=true;if(connection!=HCI_CON_HANDLE_INVALID)att_server_request_can_send_now_event(connection);}
static int uart_provision(const char*text,size_t length){char ssid[33],password[65];if(!wifi_uart_parse(text,length,ssid,password)){uart_reply("ERR FORMAT\n");return ATT_ERROR_VALUE_NOT_ALLOWED;}if(!wifi_manager_provisioning_open()){uart_reply("ERR PROVISIONING_CLOSED\n");return ATT_ERROR_WRITE_REQUEST_REJECTED;}if(!wifi_manager_set_ssid((const uint8_t*)ssid,strlen(ssid))||!wifi_manager_set_password((const uint8_t*)password,strlen(password))||!wifi_manager_request_connect()){uart_reply("ERR WIFI_REQUEST\n");return ATT_ERROR_WRITE_REQUEST_REJECTED;}uart_reply("OK WIFI CONNECTING\n");return 0;}
static int write_cb(hci_con_handle_t c,uint16_t handle,uint16_t mode,uint16_t offset,uint8_t*buffer,uint16_t size){(void)c;(void)mode;if(offset)return ATT_ERROR_INVALID_OFFSET;
 ...
 if(handle==ATT_CHARACTERISTIC_6e400002_b5a3_f393_e0a9_e50e24dcca9e_01_VALUE_HANDLE){bool single=uart_rx_len==0;if(!size||uart_rx_len+size>=sizeof(uart_rx)){uart_rx_len=0;uart_reply("ERR TOO_LONG\n");return ATT_ERROR_INVALID_ATTRIBUTE_VALUE_LENGTH;}memcpy(uart_rx+uart_rx_len,buffer,size);uart_rx_len+=size;uart_rx[uart_rx_len]=0;bool terminated=uart_rx[uart_rx_len-1]=='\n'||uart_rx[uart_rx_len-1]=='\r';bool complete=strstr(uart_rx,";PASSWORD:")!=NULL;if(!terminated&&!(single&&complete))return 0;int result=uart_provision(uart_rx,uart_rx_len);uart_rx_len=0;return result;}
 return 0;}
static void packet_handler(uint8_t type,uint16_t channel,uint8_t*packet,uint16_t size){(void)channel;(void)size;if(type!=HCI_EVENT_PACKET)return;switch(hci_event_packet_get_type(packet)){
 ...
 case ATT_EVENT_CAN_SEND_NOW:if(connection!=HCI_CON_HANDLE_INVALID){if(uart_notify_pending){uart_notify_pending=false;att_server_notify(connection,ATT_CHARACTERISTIC_6e400003_b5a3_f393_e0a9_e50e24dcca9e_01_VALUE_HANDLE,(uint8_t*)uart_tx,uart_tx_len);if(notify_pending)att_server_request_can_send_now_event(connection);}else if(notify_pending){notify_pending=false;att_server_notify(connection,ATT_CHARACTERISTIC_7e400003_b5a3_f393_e0a9_e50e24dcca9e_01_VALUE_HANDLE,(uint8_t*)status_json,status_len);}}break;
