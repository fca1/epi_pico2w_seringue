...
static void packet_handler(uint8_t type,uint16_t channel,uint8_t*packet,uint16_t size){(void)channel;(void)size;if(type!=HCI_EVENT_PACKET)return;switch(hci_event_packet_get_type(packet)){
 ...
 case ATT_EVENT_CAN_SEND_NOW:if(connection!=HCI_CON_HANDLE_INVALID&&uart_notify_pending){uart_notify_pending=false;att_server_notify(connection,ATT_CHARACTERISTIC_6e400003_b5a3_f393_e0a9_e50e24dcca9e_01_VALUE_HANDLE,(uint8_t*)uart_tx,uart_tx_len);}break;
