...
static btstack_timer_source_t idle_timer;
...
static void idle_timeout(btstack_timer_source_t *timer){(void)timer;if(connection!=HCI_CON_HANDLE_INVALID)gap_disconnect(connection);}
static void idle_timer_restart(void){btstack_run_loop_remove_timer(&idle_timer);btstack_run_loop_set_timer(&idle_timer,NUS_IDLE_TIMEOUT_MS);btstack_run_loop_add_timer(&idle_timer);}
...
static void packet_handler(uint8_t type,uint16_t channel,uint8_t*packet,uint16_t size){(void)channel;(void)size;if(type!=HCI_EVENT_PACKET)return;switch(hci_event_packet_get_type(packet)){
 ...
 case HCI_EVENT_META_GAP:if(hci_event_gap_meta_get_subevent_code(packet)==GAP_SUBEVENT_LE_CONNECTION_COMPLETE){connection=gap_subevent_le_connection_complete_get_connection_handle(packet);idle_timer_restart();}break;
 case ATT_EVENT_CAN_SEND_NOW:if(connection!=HCI_CON_HANDLE_INVALID&&uart_notify_pending){uint16_t mtu=att_server_get_mtu(connection),left=uart_tx_len-uart_tx_offset,chunk=mtu>3?mtu-3:20;if(chunk>left)chunk=left;att_server_notify(connection,ATT_CHARACTERISTIC_6e400003_b5a3_f393_e0a9_e50e24dcca9e_01_VALUE_HANDLE,(uint8_t*)uart_tx+uart_tx_offset,chunk);idle_timer_restart();uart_tx_offset+=chunk;uart_notify_pending=uart_tx_offset<uart_tx_len;if(uart_notify_pending)att_server_request_can_send_now_event(connection);}break;
 case HCI_EVENT_DISCONNECTION_COMPLETE:btstack_run_loop_remove_timer(&idle_timer);connection=HCI_CON_HANDLE_INVALID;uart_rx_len=0;uart_notify_pending=false;pending=(machine_command_t){CMD_STOP};has_command=true;gap_advertisements_enable(1);break;default:break;}}
bool ble_service_init(void){if(cyw43_arch_init())return false;pico_unique_board_id_t id;pico_get_unique_board_id(&id);for(unsigned i=0;i<6;i++)static_random_address[i]=id.id[i+2];static_random_address[0]|=0xc0;static_random_address[5]^=0x01;/* NUS-only profile identity */l2cap_init();sm_init();btstack_run_loop_set_timer_handler(&idle_timer,idle_timeout);gap_random_address_set(static_random_address);att_server_init(profile_data,read_cb,write_cb);hci_event_callback.callback=&packet_handler;hci_add_event_handler(&hci_event_callback);att_server_register_packet_handler(packet_handler);hci_power_control(HCI_POWER_ON);return true;}
