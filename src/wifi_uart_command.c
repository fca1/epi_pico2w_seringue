...
bool wifi_uart_parse(const char*text,size_t length,char ssid[33],char password[65]){
 ...
 for(size_t i=5;i+10<=length;i++)if(!memcmp(text+i,";PASSWORD:",10)){separator=text+i;break;}
 if(!separator)return false;
 size_t ssid_len=(size_t)(separator-(text+5));
 size_t password_len=length-(size_t)(separator+10-text);
 ...
 memcpy(ssid,text+5,ssid_len);ssid[ssid_len]=0;
 memcpy(password,separator+10,password_len);password[password_len]=0;
 return true;
}
