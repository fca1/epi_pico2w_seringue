...
bool command_parse_json(const char*j,size_t len,machine_command_t*o){if(!j||!o||!len||len>511)return false;char b[512];memcpy(b,j,len);b[len]=0;*o=(machine_command_t){0};
 struct{const char*n;command_kind_t k;}map[]={{"push_start",CMD_PUSH_START},{"push_stop",CMD_PUSH_STOP},{"pull_start",CMD_PULL_START},{"pull_stop",CMD_PULL_STOP},{"stop",CMD_STOP},{"dose",CMD_DOSE},{"move_relative",CMD_MOVE_RELATIVE},{"set_zero",CMD_SET_ZERO},{"reset",CMD_RESET}};
